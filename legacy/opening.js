...
(() => {
  ...
  function reveal() {
    ...
    // Slow image decoding must not leave a fade cut short by the rescue timer.
    const remaining = Math.min(1800, 2800 - performance.now() + began);
    if (remaining < 300) { finish(); return; }
    const pace = remaining / 1800;
    started = true;
    overlay.style.transitionDuration = `${800 * pace}ms`;
    overlay.classList.add('is-leaving');
    const canvas = document.querySelector('.hero-canvas');
    animate(canvas, [{ opacity: 0 }, { opacity: 1 }], { duration: 1600 * pace });
    document.querySelectorAll('.hero-copy, .hero-badges li').forEach((element, index) => {
      animate(element, [{ opacity: 0 }, { opacity: 1 }],
        { duration: 1100 * pace, delay: 160 * (index + 1) * pace });
    });
