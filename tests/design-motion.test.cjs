...
function environment(opts={}){
  ...
  media.matches=!!opts.reduce;doc.hidden=!!opts.hidden;doc.documentElement=new Element();
  ...
  doc.querySelector=()=>canvas;doc.querySelectorAll=()=>[above,...cards];doc.createElement=()=>new Element();doc.getElementById=id=>id==='services'?section:null;
  class Observer {
...
e.advance(1000);assert(e.opening());e.advance(250);assert(!e.opening(),'timer finishes even without CSS events or image decoding');assert(e.canvas.classList.contains('is-floating'));
...
e.observers[1].trigger(e.cards);assert.equal(e.animations.length,3);assert.deepEqual(e.animations.map(a=>a.options.delay),[0,60,120]);
for(const a of e.animations){assert.equal(a.options.duration,400);a.onfinish();assert(a.cancelled)}
...
e=environment({animationError:true});e.observers[1].trigger(e.cards);assert(e.cards.every(c=>!c.classList.contains('reveal-pending')));
e=environment({noObserver:true});e.advance(1250);assert(!e.opening());assert(e.cards.every(c=>!c.classList.contains('reveal-pending')));
